#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Perform integration tests for `orion.algo.hyperopt`."""
import os
import subprocess
...
def test_tpe_optimizer(database, monkeypatch):
    ...
    process = subprocess.Popen(["orion", "--config", "./orion_config_tpe.yaml",
                                "./rosenbrock.py", "-x~uniform(-5, 5, shape=3)"])
    rcode = process.wait()
    assert rcode == 0
    present_results(database, "orion_hyperopt_tpe_test")
